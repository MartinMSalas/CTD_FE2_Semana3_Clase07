...
function mostrarDatosEnPerfil() {
  ...
  //document.getElementById('cant-albums').innerText(`En total ${albumesFamosos.length} albumes`)
  let prueba = document.getElementById('cant-albums');
  console.log(prueba)
  prueba.innerText = `En total ${albumesFamosos.length} albumes ` ;
}

//<span id="cant-albums">0 álbumes</span> en tu listado - En total
//          <span id="cant-favoritos">0 favoritos</span>
//<div class="perfil">
function contarLike (albumesFamosos){
  let contador = 0;
   for (let i=0; i<albumesFamosos.length; i++){
    contador++;
  }

  console.log(albumesFamosos.length);
  /* 
